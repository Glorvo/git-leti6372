...
namespace DiskMat
{
    ...
    static class Q_5
    {
        ...
        {
            Q_6.Kostil(ref R1); //Приводим дробь к адекватному виду
            Q_6.Kostil(ref R2);

            Digit NOK = new Digit (N_14.Run(R1.Denominator.Value, R2.Denominator.Value));//Находим наибольшее общее кратное знаменателей

            //Находим, на что надо домножить числитель, чтобы привести к единому знаменателю и, собственно, домножаем
            R1.Numerator = Z_8.Run(R1.Numerator, Z_9.Run(NOK, R1.Denominator.Value));
            R2.Numerator = Z_8.Run(R2.Numerator, Z_9.Run(NOK, R2.Denominator.Value));
